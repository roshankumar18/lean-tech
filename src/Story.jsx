...
import { BsPen, BsArrowLeft, BsStar, BsShare, BsStarFill } from "react-icons/bs"
...
export default function Story() {
  ...
  const [displayCategory, setDisplayCategory] = useState('')  // use state to display category
  ...
  const [localStars, setLocalStars] = useState(
    JSON.parse(localStorage.getItem('starredStories')) || {}
  )// state for favourite category
  ...
  useEffect(() => {
    if(displayCategory) return // return when there is category in url
    if (randomCat.length > 0) { 
      const random = Math.floor((Math.random() * categories.length))
      setCheck(randomCat[random])
    }
  }, [randomCat,displayCategory])

  useEffect(()=>{
    if(!category) return
    const isValidCategory = initialCategories.some(cat=>category.toUpperCase()===cat)
    if(isValidCategory){
      setDisplayCategory(category.toUpperCase())
  ...
  //useEffect to update story count
  useEffect(()=>{
    if(categories.length>0){
      categories.forEach(cat=>{
        onValue(ref(database, `List/${cat.toUpperCase()}`), function(snapshot) {
          if (snapshot.exists()) {
            setStoryCount(prev=>({...prev,[cat.toUpperCase()]:Object.entries(snapshot.val()).length}))
          }
        })
      })
  ...
  function handleCategorySelect2(category){
    ...
    setSearchResults2([])
    searchBar(category)
    navigate(`/${category.toLowerCase()}`) // added path in browser url
  }
  ...
  function handleSubmit(e){
    ...
    if(subject && describe && selectedCategory){
      ...
      if(selectedCategory){
        ...
        toast.success('Story Posted') // added toast message
        clear()
      }
    }
  ...
  function handleClick(event){
    event.stopPropagation() // to stop  triggering global click listener
    setShow3(prev => !prev)
    setShow4(false)
  }
...
// useEffect to  close dropdowns when clicked outside
  useEffect(()=>{
    const handleGlobalClick = () =>{
      // Check if dropdown is open and close it if true
      if(show4){
        setShow4(false)
      }
  ...
  useEffect(() => {
     if(check){
      setDisplayCategory(check.toUpperCase())  // set category when category in url is not present
    ...
    if(search){
      onValue(ref(database, `List/${search.toUpperCase()}`), function(snapshot) {
        if (snapshot.exists()) {
          setStories(Object.entries(snapshot.val()).length)
          setMappable(Object.entries(snapshot.val()))
          setDisplayCategory(search.toUpperCase()) // set category when anything is searched
          navigate(`/${search.toLowerCase()}`) // append the category in url
        }else{
          navigate('/')
          setStories(0)
          setMappable([])
  ...
  function handleStarClick() {
    const starClicked =  isStarClicked
    setIsStarClicked((prev) => !prev)
    if(displayCategory){
      const newStars ={ ...localStars, [displayCategory.toUpperCase()]: !starClicked}
      setLocalStars(newStars)
      localStorage.setItem('starredStories',JSON.stringify(newStars))
    }
  }
  ...
  useEffect(()=>{
    if(displayCategory && localStars[displayCategory.toUpperCase()]!==undefined){
      setIsStarClicked(localStars[displayCategory.toUpperCase()])
    }else{
      setIsStarClicked(false)
    }
    
  },[localStars,displayCategory])
  ...
  function sorted(mappable){
    ...
    return sortedMappable.length>0 ? sortedMappable.map((items, index) => {
      const random = ((Math.random() * 4))
      return (
    ...
    }) : <div>No Story Found</div> //handle 0 story case
  }

  return (
    <div className='flex'>
      {showPopup && <Popup onClose={onClose}/>}  {/*Share Popup when share icon is clicked */}
      
      <Popular onChildValue={handleChildValue} storyCount={storyCount} isStarClicked={isStarClicked}/> 
      <div className='story-section'>
        ...
        <section className='section-2'>
          ...
          <div className='section-2-head'>
            <h1>Read stories on  <span>{displayCategory}</span></h1>   {/* Added read stories */}

            <div className='icon-container'>
              <BsStarFill className={`star-icon ${isStarClicked ? 'golden' : ''}`} onClick={handleStarClick}   // Added Star Icon
              />
            </div>    
            <div className='icon-container'>
             <BsShare  onClick={()=>setShowPopup(true)}/> {/* Added share icon */}
            </div>     
            ...
            <section className='item-section-main'>
                ...
                >
                  <div className='swiper-wrapper'>
                    ...
                    }):<div>No story found</div>   // handled 0 story case
                    })()}
                  </div>
                </Swiper>
            </section>
